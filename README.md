# Equinox logging engine 2.1.31
**Thread safety C++ logger version 2.1.31**
...
The benchmark is split by build type for comparison. The Release benchmark keeps a 10% regression guard, while the Debug benchmark uses a wider 25% guard because Debug builds are naturally noisier and more sensitive to local runtime conditions.
...
- the Debug benchmark fails when the current runtime is slower than the baseline by more than 25%
